...
// Equipment layer z-order (back to front) -- overlay layers only
const EQUIPMENT_LAYERS: { slot: string; zIndex: number }[] = [
  { slot: 'banner', zIndex: 0 },
  { slot: 'aura', zIndex: 0 },      // behind base (same z as banner)
  // base body is always at zIndex 1
  { slot: 'hairstyle', zIndex: 5 },
  { slot: 'emblem', zIndex: 7 },
  // pet and pedestal render OUTSIDE the sprite container (not overlaid)
];
...
export default function AvatarSprite({
  ...
  const petItem = equippedItems.pet ?? null;
  const pedestalItem = equippedItems.pedestal ?? null;
  const petSize = Math.round(size * 0.4);
  ...
  return (
    <div className={`avatar-sprite-container ${className}`} style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end' }}>
        ...
        >
          {/* Overlay equipment layers (banner, aura, hairstyle, emblem) */}
          {EQUIPMENT_LAYERS.map(({ slot, zIndex }) => {
        ...
        {/* Pet - outside sprite container, to the right */}
        {petItem && (
          <img
            src={getEquipmentAssetPath('pet', petItem.name)}
            alt={petItem.name}
            data-testid="equipment-pet"
            style={{
              width: `${petSize}px`,
              height: `${petSize}px`,
              imageRendering: 'pixelated',
      ...
      {/* Pedestal - equipped sprite replaces CSS pedestal */}
      {showPedestal && (
        pedestalItem ? (
          <img
            src={getEquipmentAssetPath('pedestal', pedestalItem.name)}
            alt={pedestalItem.name}
            data-testid="equipment-pedestal"
            style={{
              width: `${size}px`,
          ...
          <Pedestal level={level} size={size} />
        )
      )}
